...
void test_hal_pwm_init_apb1(void) {
  ...
  /* STM32 timer-clock rule: ×2 only when the APB prescaler is not /1. Match the
   * driver (pwm.c) so the expected values hold at any clock state — including
   * the reset HSI with APB /1, which this suite runs at. */
  uint32_t ppre1 = ((RCC->CFGR) >> RCC_CFGR_PPRE1_BIT) & 0x7;
  uint32_t timer_clk = (ppre1 == 0) ? bus_clk : bus_clk * 2;
...
void test_hal_pwm_init_apb2(void) {
  ...
  uint32_t timer_clk = (ppre2 == 0) ? bus_clk : bus_clk * 2;
  uint32_t expected_psc = timer_clk / 1000000 - 1;
