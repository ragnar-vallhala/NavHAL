...
void uart6_init(uint32_t baudrate)
{
  ...
  RCC_APB2ENR |= RCC_APB2ENR_USART6EN; // Enable USART6 clock
  USART6_BRR = (hal_clock_get_apb2clk() + (baudrate / 2)) /
               baudrate;      // BRR calculation with rounding
  USART6_CR1 = USART_CR1_TE;  // Enable transmitter
  USART6_CR1 |= USART_CR1_UE; // Enable USART
}
...
void uart2_init(uint32_t baudrate)
{
  ...
  USART2_BRR = (hal_clock_get_apb1clk() + (baudrate / 2)) /
               baudrate;                    // BRR calculation with rounding
  USART2_CR1 = USART_CR1_TE | USART_CR1_RE; // Enable transmitter and receiver
...
void uart2_write_char(char c)
{
  ...
  USART2_DR = c; // Write data to data register
  while (!(USART2_SR & USART_SR_TXE))
    ; // Wait until TXE (Transmit Data Register Empty) flag is set
  if (c == '\n')
    USART2_DR = '\r';
 ...
 * @return Non-zero if data is available, 0 otherwise
 */
int uart1_available(void) { return (USART1_SR & USART_SR_RXNE); }
 ...
 * @return Non-zero if data is available, 0 otherwise
 */
int uart2_available(void) { return (USART2_SR & USART_SR_RXNE); }
 ...
 * @return Non-zero if data is available, 0 otherwise
 */
int uart6_available(void) { return (USART6_SR & USART_SR_RXNE); }
