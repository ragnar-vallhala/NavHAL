...
/* -------------------- F7-specific: contiguous port indexing -------------------- */

/* Unlike the F401's gpio_reg.h (which remaps the PE→PH gap), the STM32F7 has
 * contiguous ports A–H, so the port number is a straight pin>>4 and the base is
 * 0x40020000 + port*0x400. Assert that on the higher ports the F4 quirk is gone. */
void test_hal_gpio_port_indexing_is_contiguous(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32((uint32_t)GPIOF_BASE_ADDR,
                           (uint32_t)GPIO_GET_PORT(GPIO_PF00));
  TEST_ASSERT_EQUAL_UINT32((uint32_t)GPIOG_BASE_ADDR,
                           (uint32_t)GPIO_GET_PORT(GPIO_PG00));
}
...
NAVTEST_CASE_DECL(test_hal_gpio_port_indexing_is_contiguous);
...
static const navtest_case_t gpio_cases[] = {
    ...
    NAVTEST_CASE(test_hal_gpio_port_indexing_is_contiguous),
    /* error paths */
