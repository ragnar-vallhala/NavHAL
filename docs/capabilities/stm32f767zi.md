...
| I2C_DMA           | ◐ | `i2c_f7.c`                              | `hal_i2c_read_regs_dma` — write reg pointer, then a DMA (RXDMAEN + AUTOEND) read. Register-correct by construction (proven CR2 framing + UART-style DMA hookup) but **not hardware-validated** (no bus device; Renode doesn't model I²C→DMA). |
