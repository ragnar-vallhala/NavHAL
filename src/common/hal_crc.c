 ...
 * @brief Shared public CRC layer + the portable software CRC-32/MPEG-2 backend.
 *
 * @details
 * Two things live here:
 *
 *  1. The public @c hal_crc_* dispatch (validate NULL config, forward to the
 ...
 *
 * Only a port with genuine hardware acceleration (STM32F4 with the CRC
 * peripheral) supplies its own ops; everyone else shares this one.
 */
...
hal_status_t hal_crc_reset(void) { return _hal_crc_ops.reset(); }

/* -------------------------------------------------------------------------- *
 * Portable software CRC-32/MPEG-2 (poly 0x04C11DB7, non-reflected, no final
...
static uint32_t s_sw_init = 0xFFFFFFFF;
static uint32_t s_sw_current = 0xFFFFFFFF;

hal_status_t hal_crc_sw_init(const hal_crc_config_t *cfg) {
  /* cfg is non-NULL: the public layer validated it before dispatching. */
  s_sw_init = cfg->init_value;
  s_sw_current = s_sw_init;
  return HAL_OK;
}
...
uint32_t hal_crc_sw_compute(const uint8_t *data, uint32_t len) {
  hal_crc_sw_reset();
  return hal_crc_sw_accumulate(data, len);
}
