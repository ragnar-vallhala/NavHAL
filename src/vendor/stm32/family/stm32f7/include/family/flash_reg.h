...
/**
 * @file flash_reg.h
 * @brief STM32F7 Flash memory interface definitions.
 *
 * @details
 * The ACR / KEYR / CR / SR register block and base address (0x40023C00) match
 * the STM32F4, so `clock.c`'s wait-state programming via
 * `FLASH_INTERFACE_REGISTER` / `FLASH_ACR_LATENCY_BIT` works unchanged for the
 * HSI/PLL bring-up path.
 *
 * @warning PLACEHOLDER SECTOR MAP. The sector addresses/sizes below are still
 * the **STM32F4** layout (16/64/128 KB). The F767 is a single 2 MB bank with
 * 32 KB×4, 128 KB×1, 256 KB×3 sectors. `DRV_FLASH` is disabled for this port
 * until the map is corrected and `flash.c`'s storage-sector selection is
 * re-reviewed — see docs/stm32f767zi_port_plan.md, milestone F7-4.
 */
